...
DNS (domain name system, система доменных имен) – это распредленная система, которая позволяет распространять имена хостов, ассоциированные с IP адресами, вместо того, чтобы требовать от каждого клиента синхронизировать `hosts` файл. Сервер имен (`name server`) публикует IP адрес для домена и предоставляет единую точку для обновления, в случае смены IP адреса.

## Ping 

Командой **`Ping`** можно проверить, доступна ли удаленная машина/сервер или нет.
...
Reply from 192.168.1.11: bytes=32 time=4ms TTL=117
```
Аргументом команды могут являться ip-адрес или имя хоста. Именем хоста может выступать, например, доменное имя. Но можно определить и собственные имена. Чтобы "пропинговать" хост с использованием имени вместо IP адреса – добавьте запись в файл  **`/etc/hosts`**. 
...
Можно определить сколько угодно много хостов в файле  **`/etc/hosts`** .
