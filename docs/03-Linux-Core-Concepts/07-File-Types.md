...
Концепция файла в linux-е важна: все является файлом с точки зрения Linux. Каждый объект в linux может быть представлен как файл определенного типа, напр. даже Директория – это специальный тип файла.
...
```
В зависимости от первого символа в списке атрибутов файла (напр. **`d`**`rwxr-xr-x+`), можно определить тип файла:

| Тип Файла        | Идентификатор |
|------------------|---------------|
| Directory        | d             |
| Regular File     | \-            |
| Character Device | c             |
